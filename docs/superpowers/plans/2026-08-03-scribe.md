...
- **Longest match wins, not first registered** (design §3). `scribe()` scores every
  intent and picks the widest match. Registration order must never be load-bearing:
  the catalogue grows task by task, and a first-match loop silently gives whichever
  pattern was added earliest the power to shadow every longer one added later.
- **Every generated program must pass `check()` on its own.** A program that names a
  variable it never declares is valid syntax and a runtime error, so an intent that
  operates on `xs` must also *declare* `xs`. Round-tripping is not enough — a tree can
  round-trip perfectly and still fail the gate.
- The full test suite must stay green (`1,217` existing tests + new ones).
...
# (every node type is already imported at the top of the file; do not
#  re-import mid-module — one import block, at the top, as everywhere else
#  in this package.)
...
`tests/test_scribe.py` needs `import pytest` at the top for the parametrize
decorators. Importing `operator.validate` **from a test** is fine — the
architecture rule constrains `src/matrixlang/scribe.py`, not the suite.
...
Expected: all PASS (existing 1,217 + new).
...
`try_operator` is SC-6's "+ try Operator when a key exists" — the server
sets it, so the browser never has to know whether a key is configured.
...
1. `python -m pytest -q` — full suite green (existing 1,217 + new Scribe tests).
